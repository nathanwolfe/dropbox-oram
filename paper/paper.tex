...
\textcolor{red}{mention the overall performance improvement in the abstact.
(Performance gain and storage reduction)}
...
Since the initial ORAM proposal by Goldreich and Ostrovsky \cite{G87, O90, GO96}, researchers have strived to improve the efficiency and reduce the trusted storage requirement of ORAM \cite{OS97, WS08, BMP11, WS12, GM11, GMOT12, Kush12, SSS12, SCSL11, GENTRY13, PathORAM} for three decade. 
Of particular interests among them is Path ORAM \cite{PathORAM}.
...
Finally, we propose a dynamically growing and shrinking tree, providing the necessary storage size flexibility to work with Dropbox.
...
We summarize how Path ORAM works in {\bf Figure \ref{fig:bintree}}. The reader can refer to \cite{PathORAM} for more details. The binary tree is stored on the server. 
...
To demonstrate the implementation of our dynamic tree structure, we tested the impact of random file addition or deletion to the tree structure. Thirty-two actions were performed using files of random sizes for this demonstration. {\bf Figure \ref{fig:resizing}}  displays the impact of an increasing and decreasing total file size stored in the tree. Whenever the tree utilization breaks the threshold set at $0.45$ or $0.55$, the tree to resizes to fulfill the target 50\% utilization.
